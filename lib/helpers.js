...
function pathify(path, pathParams) {
  ...
  // Generate test case uniq ID
  var testCaseId = `${this.endPointName}_${this.operation}_${this.responseCode}_${this.testCaseId}`;
  ...
  if (Object.keys(pathParams).length > 0) {
    ...
    for (i = 0; i < matches.length; i++) {
      ...
      if (typeof (pathParams[match]) !== 'undefined' && pathParams[match] !== null) {
        ...
        path = path.replace(re2, `{${testCaseId}}`);
      }
    }
    return path;
  }
  ...
  while(m) {
    testCaseParamId = `{${testCaseId}_${idx++}}`;
    path = `${path.substr(0, m.index)}${testCaseParamId}${path.substr(m.index + m[0].length)}`;
    m = r.exec(path);
  }
